    ...
    <div class="modal-content animate__animated animate__zoomIn">
      ...
      <div class="modal-body">
        <form @submit.prevent="submit">
          <div class="mb-4">
            ...
            <div class="photo-upload-container">
              <div class="current-photo mb-3" v-if="formData.photo">
                <p class="small text-muted mb-2">Текущее фото:</p>
                <img
                    :src="formData.photo"
                    :alt="formData.title"
          ...
          <div class="mb-3">
            <label for="editProductTitle" class="form-label">Название продукта</label>
            <input
          ...
          <div class="mb-3">
            <label for="editProductType" class="form-label">Тип продукта</label>
            <select
          ...
          <div class="mb-3">
            <label for="editProductSpecification" class="form-label">Характеристики</label>
            <textarea
                v-model="formData.specification"
                class="form-control"
                id="editProductSpecification"
                rows="3"
          ...
          <div class="mb-3">
            <label for="editProductSerialNumber" class="form-label">Серийный номер</label>
            <input
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="mb-3">
                <label for="editProductGuaranteeStart" class="form-label">Начало гарантии</label>
                <input
                    v-model="formData.guaranteeStart"
                    type="date"
                    class="form-control"
                    id="editProductGuaranteeStart"
                    required
                >
              </div>
            </div>
            <div class="col-md-6">
              <div class="mb-3">
                <label for="editProductGuaranteeEnd" class="form-label">Окончание гарантии</label>
                <input
                    v-model="formData.guaranteeEnd"
                    type="date"
                    class="form-control"
                    id="editProductGuaranteeEnd"
                    required
                >
              </div>
            </div>
...
watch(() => props.show, (newVal) => {
  if (newVal && props.product) {
    ...
    const defaultPriceIndex = product.price.findIndex((p: Price) => p.isDefault)
    ...
    formData.value = {
      ...
      serialNumber: product.serialNumber || 0,
      guaranteeStart: product.guarantee?.start ? new Date(product.guarantee.start).toISOString().split('T')[0] : '',
      guaranteeEnd: product.guarantee?.end ? new Date(product.guarantee.end).toISOString().split('T')[0] : '',
      prices: product.price?.map((p: Price) => ({
        value: p.value || 0,
        symbol: p.symbol || 'UAH',
        isDefault: p.isDefault || false
      })) || [],
      defaultPriceIndex: defaultPriceIndex >= 0 ? defaultPriceIndex : 0,
...
const removeUploadedFile = () => {
  ...
  if (props.product) {
    formData.value.photo = props.product.photo || ''
  }
}
...
const submit = () => {
  if (!formData.value.title.trim() || !props.product) return
